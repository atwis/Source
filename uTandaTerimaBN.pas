...
uses
  ...
  cxDBLookupComboBox, dxSkinsCore, dxSkinsDefaultPainters, ImgList, Grids,
  DBGrids, CRGrid;

type
  TfTandaTerimaBN = class(TForm)
    Label5: TLabel;
    Shape2: TShape;
    Label3: TLabel;
...
var
  fTandaTerimaBN: TfTandaTerimaBN;
...
procedure TfTandaTerimaBN.FormResize(Sender: TObject);
var
  tLebar : Integer;
begin
...
procedure TfTandaTerimaBN.RefreshAwal;
begin
  with QBiru do
  begin
    Close;
...
procedure TfTandaTerimaBN.RefreshPemohon;
begin
  UniQuery1.Close;
...
procedure TfTandaTerimaBN.TampilAwal;
begin
  dtTglPrintTTBBN1.Date := TTglSrvSkr;
  cbPemohonTTBBN1.Text := '';
end;

procedure TfTandaTerimaBN.CRDBGrid1DrawColumnCell(Sender: TObject;
...
procedure TfTandaTerimaBN.CRDBGrid2DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
var
  bitmap : TBitmap;
...
procedure TfTandaTerimaBN.RefreshPilih;
begin
  with QHijau do
  begin
    Close;
...
procedure TfTandaTerimaBN.cxButton2Click(Sender: TObject);
begin
...
procedure TfTandaTerimaBN.cxButton3Click(Sender: TObject);
begin
  if (QBiru.Active) and (QBiru.RecordCount > 0) then
  begin
...
procedure TfTandaTerimaBN.cxButton4Click(Sender: TObject);
begin
...
procedure TfTandaTerimaBN.cxButton5Click(Sender: TObject);
begin
  if (QHijau.Active) and (QHijau.RecordCount > 0) then
  begin
    UniQuery3.Close;
...
procedure TfTandaTerimaBN.frxReportAfterPrint(Sender: TfrxReportComponent);
begin
  with DM.Q do
  begin
...
procedure TfTandaTerimaBN.cxButton1Click(Sender: TObject);
begin
  if (QHijau.Active) and (QHijau.RecordCount > 0) then
  begin
...
procedure TfTandaTerimaBN.CRDBGrid1DblClick(Sender: TObject);
begin
  cxButton3.Click;
end;

procedure TfTandaTerimaBN.FormShow(Sender: TObject);
var
  tLebar : Integer;
begin
  tLebar := Round((Width-69)/2);
...
procedure TfTandaTerimaBN.bGetDataKIClick(Sender: TObject);
begin
  with uniSPBiru do
  begin
    Active := False;
    StoredProcName := 'bpkb_get_list_tt_bn';
    try
      Params.Clear;
...
procedure TfTandaTerimaBN.cxButton8Click(Sender: TObject);
begin
  RefreshPemohon;
end;
