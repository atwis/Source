...
uses
  ...
  dxSkinscxPCPainter, Grids, DBGrids, CRGrid;

type
  TfPenyerahanBPKBBN = class(TForm)
    Label1: TLabel;
    Shape1: TShape;
    Shape2: TShape;
    Label3: TLabel;
...
var
  fPenyerahanBPKBBN: TfPenyerahanBPKBBN;
...
procedure TfPenyerahanBPKBBN.CariData(vCari: String);
begin
  with DM.Q do
  begin
...
procedure TfPenyerahanBPKBBN.HapusDataTemp;
begin
  with DM.Q do
  begin
    Close;
    SQL.Text := 'DELETE FROM T_PENYERAHAN_TEMP WHERE SNHDD=:SNHDD AND PENYERAHAN_ID=1';
    try
...
procedure TfPenyerahanBPKBBN.RefreshDataTemp;
begin
  with Q do
  begin
    Close;
    SQL.Text := 'SELECT B.SNHDD,B.PENYERAHAN_ID,B.PENYERAHAN_TGL,B.OP_ID,B.PENYERAHAN_NOREG,B.PEMOHON_ID,B.BPKB_ID,B.PENYERAHAN_KET,'+
                ...
                ' INNER JOIN M_PEMOHON pmh ON pmh.PEMOHON_ID = df.PEMOHON_ID'+
                ' INNER JOIN t_OPERATOR c ON c.OP_ID = B.OP_ID'+
                ' WHERE B.SNHDD=:SNHDD AND B.PENYERAHAN_ID=1';
    try
      Params[0].AsString := tSNHDD;
      Open;
    except on E: Exception do
...
procedure TfPenyerahanBPKBBN.RefrezshPemohon;
begin
  UniQuery1.Close;
...
procedure TfPenyerahanBPKBBN.SimpanData;
var
  Temp : String;
begin
  with DM.Q do
...
procedure TfPenyerahanBPKBBN.TampilAwal;
begin
  RefrezshPemohon;
  HapusDataTemp;
  RefreshDataTemp;
...
procedure TfPenyerahanBPKBBN.UpdateTChekPoint(vKode, vOPID, vTgl: String);
begin
  with DM.Q1 do
  begin
    Close;
...
procedure TfPenyerahanBPKBBN.bBaruClick(Sender: TObject);
begin
  TampilAwal;
end;

procedure TfPenyerahanBPKBBN.bGetDataBPKBClick(Sender: TObject);
begin
...
procedure TfPenyerahanBPKBBN.CRDBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  if dsQ.DataSet.RecNo > 0 then
  begin
  if Column.Title.Caption = 'No' then
     CRDBGrid1.Canvas.TextOut(Rect.Left + 2, Rect.Top, IntToStr(dsQ.DataSet.RecNo));
  end;
end;

procedure TfPenyerahanBPKBBN.bSimpanSerahClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      try
        ParamByName('SNHDD').AsString := tSNHDD;
        ParamByName('PENYERAHAN_ID').AsInteger := 1;
...
procedure TfPenyerahanBPKBBN.cxButton1Click(Sender: TObject);
begin
...
procedure TfPenyerahanBPKBBN.FormShow(Sender: TObject);
begin
  TampilAwal;
  RefrezshPemohon;
...
procedure TfPenyerahanBPKBBN.frxReportGetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TfPenyerahanBPKBBN.cxButton8Click(Sender: TObject);
begin
  RefrezshPemohon;
end;
