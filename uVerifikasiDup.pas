...
var
  fVerifikasiDup: TfVerifikasiDup;
...
procedure TfVerifikasiDup.CariData;
var
...
procedure TfVerifikasiDup.TampilAwal;
begin
  vIsBBN := '5';
  eNoBPKB_V.Text := '';
  eNoRangka_V.Text := '';
...
procedure TfVerifikasiDup.UpdateVerfikasi;
var
  uniSP : TUniStoredProc;
begin
  try
...
procedure TfVerifikasiDup.FormShow(Sender: TObject);
begin
  dtTgl.Date := TTglSrvSkr;
  TampilAwal;
end;

procedure TfVerifikasiDup.cxButton1Click(Sender: TObject);
begin
...
procedure TfVerifikasiDup.bBaru_VClick(Sender: TObject);
begin
  TampilAwal;
end;

procedure TfVerifikasiDup.bVerifikasiClick(Sender: TObject);
begin
  if MessageDlg('YAKIN DATA AKAN DI VERIFIKASI?',mtInformation,[mbYes,mbNo],0)=mrYes then
     UpdateVerfikasi;
end;

procedure TfVerifikasiDup.eNoBPKB_VKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 13 then
  begin
...
procedure TfVerifikasiDup.eNoRangka_VKeyDown(Sender: TObject;
...
procedure TfVerifikasiDup.eNoBarcode_VKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if Key = 13 then
  begin
