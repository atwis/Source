...
uses
  ...
  cxDataStorage, DB, cxDBData, cxCheckBox, MemDS, DBAccess, Uni,
  ...
  cxCalendar, cxButtons, cxTextEdit, ExtCtrls, frxClass, frxDBSet,
  frxBarcode, frxDesgn, Grids, DBGrids, CRGrid;

type
  TfPendaftaranBN = class(TForm)
    Label2: TLabel;
    Label3: TLabel;
    Bevel1: TBevel;
    Label4: TLabel;
...
var
  fPendaftaranBN: TfPendaftaranBN;
...
procedure TfPendaftaranBN.RefreshCbPemohon;
begin
  UniQuery1.Close;
...
procedure TfPendaftaranBN.RefreshData;
begin
  with uniSPTampil do
  begin
    Active := False;
    StoredProcName := 'list_pendaftaran_bn';
    try
      Params.Clear;
...
procedure TfPendaftaranBN.TampilAwal;
begin
  ...
  begin
    eNoResiPembayaran.Text := NoResiPembayaranBank;
    eNoBPKB.SetFocus;
  end
  else
  begin
...
procedure TfPendaftaranBN.FormShow(Sender: TObject);
begin
  TampilAwal;
end;

procedure TfPendaftaranBN.rbPribadiClick(Sender: TObject);
begin
  RefreshCbPemohon;
  cbPemohon.SetFocus;
end;

procedure TfPendaftaranBN.rbBJClick(Sender: TObject);
begin
  RefreshCbPemohon;
  cbPemohon.SetFocus;
end;

procedure TfPendaftaranBN.bTampilDataClick(Sender: TObject);
begin
...
procedure TfPendaftaranBN.bBaruPendaftaranBBN1Click(Sender: TObject);
begin
  RefreshData;
  lRoda.Caption := '';
  //vIsBBN := '1';
...
procedure TfPendaftaranBN.bSimpanClick(Sender: TObject);
var
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if MessageDlg('Yakin Data Pendaftaran Berkas Balik Nama Akan Di-Rubah?',mtConfirmation,[mbYes,mbNo],0)=mrYes then
                begin
                  try
                    fWait.Show;
                    ...
                    uniSP.Close;
                    uniSP.StoredProcName := 'bpkb_pendaftaran_edit_bn';
                    try
                      uniSP.Params.Clear;
                      ...
                      uniSP.Params.CreateParam(ftString,'vEnrollmentType',ptInput).AsInteger := 2;
              ...
              begin
                if MessageDlg('Yakin Data Pendaftaran Berkas Balik Nama Akan Di-Simpan?',mtConfirmation,[mbYes,mbNo],0)=mrYes then
                begin
                  try
                    fWait.Show;
                    Application.ProcessMessages;
                    ...
                    try
                      ...
                      uniSP.Params.CreateParam(ftString,'vPetugasID',ptInput).AsString := vIDPetugas;
                      if TKODEBAYAR='1' then
                         uniSP.Params.CreateParam(ftString,'vBarcodeBank',ptInput).AsString := TKODEBANK+'2'+TLoket+FormatDateTime('YYMMDD',TTglSrvSkr)+CekNoUrut
                      else
                         uniSP.Params.CreateParam(ftString,'vBarcodeBank',ptInput).AsString := eNoResiPembayaran.Text;
                      uniSP.Params.CreateParam(ftString,'vLoketNo',ptInput).AsString := TLoket;
                      uniSP.Params.CreateParam(ftString,'vEnrollmentType',ptInput).AsInteger := 2;
                      ...
                      begin
                        Log('***START FOR TRACE LOG ADD BPKB - BALIK NAMA - '+eNoBPKB.Text+'***');
                      end;
                      uniSP.ExecProc;
                      fWait.Hide;
                      if Copy(uniSP.Params.ParamValues['RESULT'],1,2)<>'00' then
...
procedure TfPendaftaranBN.bHapusClick(Sender: TObject);
var
  uniSP : TUniStoredProc;
begin
  if lEdit.Caption <> '' then
    ...
    begin
      ...
      begin
        try
          ...
          uniSP.Connection := DM.dbSrv;
          uniSP.Close;
          uniSP.StoredProcName := 'bpkb_pendaftaran_delete_bn';
          try
            uniSP.Params.Clear;
...
procedure TfPendaftaranBN.bCetakClick(Sender: TObject);
begin
...
procedure TfPendaftaranBN.EDITDATA1Click(Sender: TObject);
begin
  if (uniSPTampil.Active) and (uniSPTampil.RecordCount > 0) then
  begin
    if uniSPTampil.FieldByName('Sudah_Diproses').AsInteger = 1 then
...
procedure TfPendaftaranBN.HAPUSDATA1Click(Sender: TObject);
var
  uniSP : TUniStoredProc;
begin
  if uniSPTampil.FieldByName('Sudah_Diproses').AsInteger <> 1 then
...
procedure TfPendaftaranBN.frxReportGetValue(const VarName: String;
  var Value: Variant);
begin
...
function TfPendaftaranBN.NoResiPembayaranBank: String;
var
  uniSP : TUniStoredProc;
begin
  try
...
function TfPendaftaranBN.CekNoUrut: String;
begin
  Result:= '0000';
  with DM.Q do
  begin
...
procedure TfPendaftaranBN.UpdateNoUrut;
begin
  with DM.Q do
  begin
    Close;
...
procedure TfPendaftaranBN.cxGridDBTableView1StylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; out AStyle: TcxStyle);
begin
